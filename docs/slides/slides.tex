...
\begin{frame}{Memory Model}
  ...
  \textbf{local to each loop iteration} — in both individual and group mode.

  \vspace{0.8em}
  Storage is determined by how the variable is used:
  \begin{itemize}
    \item \textbf{Registers} — variable is always used as a whole (never indexed element-by-element)
    \item \textbf{Shared memory} — variable is ever indexed; implies one thread may read
          a value written by another thread, which requires shared memory
  \end{itemize}

  \vspace{0.8em}
  \begin{alertblock}{Individual mode exception}
    Only one thread owns each iteration — no inter-thread sharing.
    All variables stay in \textbf{registers}, even if indexed.
    Large arrays spill transparently via the backend compiler.
  \end{alertblock}
