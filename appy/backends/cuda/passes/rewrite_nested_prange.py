    ...
    def visit_For(self, node):
        if self.is_iter_prange(node.iter):
            ...
            if isinstance(firstchild, ast.For) and self.is_iter_prange(firstchild.iter):
                assert len(node.iter.args) == 1 and len(firstchild.iter.args) == 1, \
                    f"Only prange loops with 1 argument are supported, got: {ast.unparse(node)}"
                ...
                new_loop = ast.For(
                    ...
                    iter=ast.Call(
                        ...
                        args=[ast.BinOp(op=ast.Mult(), left=outer_bound, right=inner_bound)],
                        keywords=[]
                    ),
                    body=[]
                )
                new_loop.body = [
                    ast.Assign(
                        ...
                        value=ast.BinOp(op=ast.FloorDiv(),
