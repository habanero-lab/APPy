...
def kernel_appy2(TSTEPS, A, B):
    for t in range(1, TSTEPS):
        ...
        #pragma parallel for
        for i in range(1, B.shape[0] - 1):
            A[i, 1:-1] = 0.2 * (B[i, 1:-1] + B[i, :-2] + B[i, 2:] +
                                 B[i - 1, 1:-1] + B[i + 1, 1:-1])
    return A, B
...
def test():
    ...
    A2, B2 = nps.copy(A), nps.copy(B)
    kernel_appy(10, A2, B2)

    A3, B3 = nps.copy(A), nps.copy(B)
    kernel_appy2(10, A3, B3)

    assert np.allclose(A1, A2, atol=1e-5)
    assert np.allclose(B1, B2, atol=1e-5)
    assert np.allclose(A1, A3, atol=1e-5)
    assert np.allclose(B1, B3, atol=1e-5)
